...
なんか自宅にサーバー欲しくなるんですよね。何故か 1TB の SSD が転がっていたので 6 千円程度で Fujitu のデスクトップ PC を買ってサーバー機にしています。逸般の誤家庭ではないのでラックサーバーなんてものはありません。
Proxmox を入れてその上に Ubuntu Server をぶち込み、Samba を使っています。ファイルサーバーとしては十分な動きをしてくれているのでこれでいいはずなのですが、おうち k8s をやりたくなっており新しくマシンを買う機運が高まっているこの日この頃です。
...
パソコンでゲームをしないので、デスクトップ PC を組む必要がなく常にこれを使って生活してます。
...
貧弱な耳なので、秋葉のどこかの店で買った 2000 円くらいのスピーカーで満足してます。本当はスピーカーの沼に足を踏み入れたいのですが、チキっています。
...
マウス操作、面倒じゃありません？トラックポイントを使うとホームポジションを崩さずにカーソルが操作できるんです。そこでトラックポイントがついた、ThinkPad トラックポイントキーボード II を購入しました。
（この記事を書いている途中で届きました。）
...
他の端末で音楽を再生できるのが非常に強いです。クロスプラットホームなので、スマホからパソコン（Linux）で再生するなんてことが簡単にできます。Spotify Premium 入りましょう。

## uLauncher

主に Linux 向けのランチャーアプリです。macOS だと Raycast とか Alfred とかがあるのですが、Linux でランチャーアプリ使うならこれ一択です。マジで便利です。（が、Wayland を使ってると上手く動かないことがあるので注意）
...
色々紹介してきましたが、この環境を構築してから、本当に作業効率が上がりました。（銀行の口座残高は下がりました）
やはり自分の気に入った環境で開発などができると本当に効率が違います。ぜひ皆さん自分の好みの環境を整えましょう！！

ps.
記事を書いてたらなんか知らないけど 12 月になってました
